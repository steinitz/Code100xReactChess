...
const Square = React.memo(({ white, row, col, letters, numbers }: SquareProps) => {
  ...
  return (
    ...
    >
      {numbers && <Text style={[textStyle, { opacity: newLocal ? 1 : 0 }]}>{'' + rowNumber}</Text>}
      {row === 7 && letters && (
        <Text style={[textStyle, { alignSelf: 'flex-end' }]}>{String.fromCharCode(97 + colNumber)}</Text>
      )}
    </View>
  );
});
  ...
  return (
    <View style={styles.container}>
      {new Array(8).fill(0).map((_, i) => (
        <Square {...rest} row={row} col={i} key={i} white={(i + offset) % 2 === 1} />
      ))}
    </View>
  );
});
...
const Background: React.FC = React.memo(() => {
  ...
  return (
    ...
    >
      {new Array(8).fill(0).map((_, i) => (
        <Row key={i} white={i % 2 === 0} row={i} letters={withLetters} numbers={withNumbers} />
      ))}
    </View>
  );
});
